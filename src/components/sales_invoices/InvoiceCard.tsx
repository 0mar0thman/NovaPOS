...
const InvoiceCard = ({ invoice, getPaymentStatusColor, getPaymentStatusText, formatDate, currentUser, onClick, onPrint }: InvoiceCardProps) => {
  return (
    <Card
      className={`border-blue-200 dark:border-slate-600 hover:shadow-md dark:hover:shadow-slate-700 transition-all duration-200 cursor-pointer w-full ${
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 sm:gap-4">
          <div className="space-y-2 sm:flex-1">
            <div className="flex flex-wrap items-center gap-2">
              <Badge variant="outline" className="text-blue-600 dark:text-blue-400 border-blue-300 dark:border-blue-500 text-xs sm:text-sm">
              ...
              {invoice.customer_name && (
                <Badge variant="outline" className={
                  invoice.customer_name === 'عميل فوري' 
                    ? 'text-purple-600 dark:text-purple-400 border-purple-300 dark:border-purple-500 text-xs sm:text-sm' 
                    : 'text-gray-600 dark:text-gray-400 border-gray-300 dark:border-gray-500 text-xs sm:text-sm'
                }>
                  {invoice.customer_name}
                </Badge>
              )}
          ...
          <div className="text-left sm:text-right">
            ...
            <Button 
              variant="ghost" 
              size="sm" 
              className="mt-1 sm:mt-2 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-700 text-xs sm:text-sm px-2 sm:px-3"
              onClick={(e) => {
                e.stopPropagation();
                onPrint();
              }}
